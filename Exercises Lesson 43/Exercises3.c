// Viết chương trình quản lí nhân viên sử dụng typedef
...
void addNewStaff(Staff* s);
// hàm xóa nhân viên theo mã
int removeStaff(Staff* staffs, size_t* n, char* id);
// hàm cập nhật chấm công cho nhân viên
int updateDOW(Staff* s, size_t n, char* id, int dow);
// hàm tính lương và bonus
void calculateTotalIncome(Staff* staffs, size_t n);
// hàm sắp xếp danh sách nhân viên theo tổng lương giảm dần
void sortByIncome(Staff* staffs, size_t n);
// hàm sắp xếp danh sách nhân viên theo tên tăng dần
void sortByName(Staff* staffs, size_t n);
// hàm hiển thị danh sách nhân viên hiện có ra màn hình
void listedStaffMembers(const Staff* staffs, size_t n);
// hàm đổi chỗ hai nhân viên
void swap(Staff* a, Staff* b);
// hàm hiển thị 1 nhân viên
void showAStaff(Staff s);

int main() {
	int choice;
	Staff staffs[100]; // giả sử có tối đa 100 sv
...
int updateDOW(Staff* staffs, size_t n, char* id, int dow) {
	size_t i;
	for (i = 0; i < n; i++) {
		// nếu tìm thấy 
		if (strcmp(staffs[i].id, id) == 0) {
