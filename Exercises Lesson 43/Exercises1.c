// Quản lí tài khoản ngân hàng sử dụng typedef
...
// hàm nguyên mẫu
// hàm thêm mới một tk
void createAccount(BankAcc* accounts);
// hàm cập nhật số dư theo mã tk
void updateBallance(BankAcc* acc, long newBallance);
// hàm hiển thị danh sách tk
void listedBankAcc(const BankAcc* accounts, size_t n);
// hàm xóa tài khoản theo mã
void removeAccount(BankAcc* accounts, size_t* n, int id);
// hàm tìm một tk theo mã
int findAccById(const BankAcc* accounts, size_t n, int id);
// hàm sắp xếp danh sách tài khoản ngân hàng theo số dư
void sortAccByBallance(BankAcc* accounts, size_t n);
// hàm hiển thị 01 bản ghi
void showAnAcc(BankAcc acc);

int main() {
	int choice;
	BankAcc accounts[100]; // giả sử có tối đa 100 tk
